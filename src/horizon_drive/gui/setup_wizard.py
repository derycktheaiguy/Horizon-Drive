...
class SetupWizard(ctk.CTkToplevel):
    def __init__(self, master, on_complete):
        ...
        self.config = {"local_folder": os.path.expanduser("~/HorizonDrive"), "sync_mode": "Two-way"}
    ...
    def _build_ui(self):
        ...
        self.title_label = ctk.CTkLabel(
            self.main_frame, text="Welcome to Horizon Drive", font=ctk.CTkFont(size=24, weight="bold")
        )
        self.title_label.pack(pady=(30, 10))

        self.subtitle_label = ctk.CTkLabel(
            self.main_frame, text="Let's configure your local cargo hold.", font=ctk.CTkFont(size=14)
        )
        ...
        self.browse_button = ctk.CTkButton(
            self.folder_inner_frame, text="Browse", width=80, command=self._browse_folder
        )
        ...
        self.mode_two_way = ctk.CTkRadioButton(
            self.main_frame, text="Two-way Sync (Cloud <-> Local)", variable=self.mode_var, value="Two-way"
        )
        self.mode_two_way.pack(pady=5, anchor="w", padx=60)

        self.mode_backup = ctk.CTkRadioButton(
            self.main_frame, text="Backup Only (Local -> Cloud)", variable=self.mode_var, value="Backup"
        )
        self.mode_backup.pack(pady=5, anchor="w", padx=60)

        # Finish
        self.finish_button = ctk.CTkButton(
            self.main_frame, text="Start Syncing", command=self._finish, height=45, font=ctk.CTkFont(weight="bold")
        )
